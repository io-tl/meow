...
The `config.yaml` file is **optional**. Without a configuration file, the grabber uses default values and attempts to connect to `nats://localhost:4222`. CLI flags always take priority.
...
-w, --workers <n>           Number of workers
    ...
    --nats-token <tok>      NATS auth token
    --probe-timeout <ms>    Per-probe timeout (default: 9000)
    --global-timeout <ms>   Global timeout per port (default: 30000)
```
...
-w, --workers <n>            Number of workers
    --nats-url <url>         NATS server URL
    --nats-token <tok>       NATS auth token
    --enrich-timeout <ms>    Per-module timeout (default: 10000)
    --global-timeout <ms>    Hard deadline per job (default: 30000)
```

### `grab local`

Combines flags from `finger` and `enrich` (`--probe-timeout`, `--enrich-timeout`, `--global-timeout`).
